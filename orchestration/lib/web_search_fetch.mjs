...
export async function runWebSearchFetch({ query, tenant = 'default', outDir = 'websearch' }) {
  ...
  if (!BRAVE_API_KEY || TEST_MODE) {
    ...
    let mockHtml =
      '<!DOCTYPE html><html><head><title>Test Page</title></head><body><h1>Test</h1></body></html>';
        ...
        results: [
          {
            ...
            description:
              'Discover our wide range of electronics, furniture, and office supplies. Mock result for TEST_MODE.',
    ...
    console.log(
      `[web_search_fetch] ✅ ${mockData.web.results[0].title} -> ${mockData.web.results[0].url}`,
    );
