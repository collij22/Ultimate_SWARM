...
describe('Quick PNG Decoder Tests', () => {
  it('should create valid PNG header', () => {
    ...
    assert.deepEqual(pngBuffer.slice(0, 8), signature, 'PNG should have valid signature');
  });
  ...
  it('should be fast to generate', () => {
    ...
    assert.ok(avgTime < 50, `Should generate PNG quickly (took ${avgTime.toFixed(2)}ms average)`);
  });
});
