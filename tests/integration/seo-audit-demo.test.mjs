...
describe('SEO Audit Demo Integration Tests', () => {
  it('should execute seo.audit capability with fixture', async () => {
    ...
    const { executeSEOAudit } = await import(
      '../../orchestration/lib/deterministic/seo_audit_executor.mjs'
    );
  ...
  it('should execute doc.generate capability', async () => {
    ...
    const { executeDocGenerate } = await import(
      '../../orchestration/lib/deterministic/doc_generate_executor.mjs'
    );
  ...
  it('should produce valid SEO metrics for Phase 11 validation', async () => {
    ...
    const brokenLinks = audit.data.links.external_samples.filter((link) =>
      link.includes('broken'),
    ).length;
    ...
    for (const tag of requiredMeta) {
      assert.ok(audit.data.meta_tags[tag], `Should have required meta tag: ${tag}`);
    }

    // Check required OG tags
    const requiredOG = ['og:title', 'og:description'];
    for (const tag of requiredOG) {
      assert.ok(audit.data.open_graph[tag], `Should have required OG tag: ${tag}`);
    }
  });
});
