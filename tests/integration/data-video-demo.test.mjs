...
describe('Data-Video Demo Integration Tests', () => {
  it('should execute data.ingest capability', async () => {
    // Test data ingestion
    const { executeDataIngest } = await import('../../orchestration/lib/deterministic/data_ingest_executor.mjs');
  ...
  it('should execute data.insights capability', async () => {
    ...
    const { executeDataInsights } = await import('../../orchestration/lib/deterministic/data_insights_executor.mjs');
  ...
  it('should execute chart.render capability', async () => {
    ...
    const { executeChartRender } = await import('../../orchestration/lib/deterministic/chart_render_executor.mjs');
    ...
    await new Promise((resolve, reject) => {
      ...
      png.on('parsed', function() {
        try {
          ...
          console.log(`PNG Content: ${colorMap.size} unique colors, ${(nonWhiteRatio * 100).toFixed(2)}% non-white`);
          
          // Validate chart has actual content
          assert.ok(colorMap.size >= 3, `Chart should have at least 3 colors (got ${colorMap.size})`);
          assert.ok(nonWhiteRatio > 0.05, `Chart should have >5% non-white pixels (got ${(nonWhiteRatio * 100).toFixed(2)}%)`);
  ...
  it('should execute audio.tts capability', async () => {
    // Test TTS generation
    const { executeAudioTTS } = await import('../../orchestration/lib/deterministic/audio_tts_executor.mjs');
  ...
  it('should execute video.compose capability', async () => {
    ...
    const { executeVideoCompose } = await import('../../orchestration/lib/deterministic/video_compose_executor.mjs');
