...
export function generateCoverageReport() {
  ...
  Object.values(policies.capability_map || {}).flat().forEach(t => mappedTools.add(t));
  ...
  for (const [toolId, tool] of Object.entries(registry.tools || {})) {
    if (tool.tier === 'secondary') {
      ...
      if (!hasOverride && tool.cost_model?.usd > (policies.tiers?.secondary?.default_budget_usd || 0.10)) {
        report.missing_budgets.push({
          tool_id: toolId,
          cost: tool.cost_model?.usd,
          default_budget: policies.tiers?.secondary?.default_budget_usd || 0.10,
  ...
  report.statistics.average_tools_per_capability = capToolCounts.length > 0 
  ...
  report.statistics.average_allowlist_size = allowlistSizes.length > 0
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  try {
    ...
    console.log('\n📊 Router Coverage Report\n');
    console.log('Summary:');
    console.log(`  Tools: ${report.summary.total_tools} (${report.statistics.primary_tools} primary, ${report.statistics.secondary_tools} secondary)`);
    ...
    if (report.orphaned_capabilities.length > 0) {
      console.log(`❌ ${report.orphaned_capabilities.length} capabilities have invalid tool references`);
    }
    
    if (report.restrictive_agents.length > 0) {
      console.log(`⚠️  ${report.restrictive_agents.length} agents have very restrictive allowlists (<3 tools)`);
    }
    
    if (report.missing_budgets.length > 0) {
      console.log(`⚠️  ${report.missing_budgets.length} expensive secondary tools lack budget overrides`);
    }
